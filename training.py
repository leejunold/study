...
def preprocessing(desease):
    ...
    X = desease[['체온', '수축기 혈압', '이완기 혈압', '호흡 곤란', '간헐성 경련', '설사', '기침', '출혈', '통증', '만지면 아프다', '무감각', '마비', '현기증', '졸도',
...
with tab1:
    ...
    with col112:
        st.info('출동시간')
    with col113:
        now_time = st.time_input('현재 시간', label_visibility='collapsed', step=60)
    ...
    with col120:
        st.info('이름')
    with col121:
        name = st.text_input('이름', label_visibility='collapsed', value='이준영')
    with col122:
        st.info('나이')
    with col123:
        age = st.number_input('나이', label_visibility='collapsed', value=28, min_value=0, max_value=120)
    with col124:
        st.info('성별')
    with col125:
        patient_s = st.radio('성별',['남성','여성'], label_visibility='collapsed')
   ...
   ##-------------------------------------------------------------------------------------
    ...
    with col130:
        st.info('체온')
    with col131:
        fever = st.number_input('체온', label_visibility='collapsed', min_value=0.0, max_value=50.0, value=36.5 , step=0.1)
    with col132:
        st.info('환자 위치')
    with col133:
        location = st.text_input('환자 위치', label_visibility='collapsed', value='대구광역시 달서구 용산로12길')
    ...
    col15, col16, col17, col18 = st.columns([1, 3, 1, 3])

    with col15: 
        st.info('수축기 혈압')

    with col16: 
        systolic_pressure = st.slider('수축기 혈압', 0, 200, label_visibility='collapsed')

    with col17: 
        st.info('이완기 혈압')

    with col18:
        diastolic_pressure = st.slider('이완기 혈압', 0, 200, label_visibility='collapsed')

    # Determine high and low blood pressure
    ...
    if high_blood == True:
        st.write(f'{name}은 고혈압입니다')
    elif low_blood ==True:
        st.write(f'{name}은 저혈압입니다.')
    else:
        st.write(f'{name}은 정상혈압입니다.')
    ...
    col19, col20, col21, col22, col23, col24, col25, col26 = st.columns(8)
    with col19: 
        st.error("증상 체크")
    with col20: 
    ...
    with col23: 
        stammer_check = st.checkbox('말이 어눌해짐')
        drowsiness_check = st.checkbox('졸도')
    with col24: 
    ...
    with col26: 
        ...
        # ('I ['기침', '마비', '통증', '말이 어눌해짐', '설사', '시력 저하', '현기증', '간헐적 경련', '무감각', '만지면 아픔', '졸도', ' 졸림', '호흡 곤란'])
    ...
    col27, col28, col29 = st.columns([1, 3, 4])
    with col27: 
        st.error("중증 질환 여부")
    with col28:
        severe_condition = st.selectbox('중증 질환 여부', ('중증 질환 있음', '잘 모르겠음', '중증 질환 없음'), label_visibility='collapsed')

    if severe_condition == '중증 질환 있음':
        special_yn = '중증 질환 선택'
    elif severe_condition == '잘 모르겠음':
        special_yn = '중증 질환 예측'
    elif severe_condition == '중증 질환 없음':
        special_yn = '중증 아님'
    ...
    col30, col31 = st.columns(2) # col 나누기
    ...
    with col30:
        if special_yn == '중증 질환 예측':
            ...
            patient_data = {
                "체온": [int(fever)],
                "수축기 혈압": [int(systolic_pressure)],
                "이완기 혈압": [int(diastolic_pressure)],
                ...
                "졸도": [int(drowsiness_check)],
                "말이 어눌해졌다": [int(stammer_check)],
            ...
            st.write("중증 질환 예측은 뇌경색, 뇌출혈, 복부손상, 심근경색 4가지만 분류됩니다.")
...
#                 ###### folium을 활용하여 지도 그리기 (3일차 교재 branca 참조)
                ...
                m2 = folium.Map(location=[lati, long], zoom_start=11)
                ...
                folium.Marker(location=[patient_lat, patient_lon], 
                              popup='환자', tooltip='환자위치', icon=icon).add_to(m2)
...
#     #  -------------------- ▼ 1-9그룹 완료시간 저장 폼 지정 ▼  --------------------
        ...
        ## 완료시간 시간표시 cols 구성
        col1, col2 = st.columns(2) # col 나누기
        
        with col1:
            st.info('완료 시간')
        with col2:
            end_time = st.time_input('시간', label_visibility='collapsed')
        ...
        if st.form_submit_button(label='저장하기'):
            ...
            if check_condition1 and check_condition2:
                ...
                dispatch_data.loc[dispatch_data['ID'] == max_id, '체온'] = int(fever)
                ...
                dispatch_data.loc[dispatch_data['ID'] == max_id, '졸도'] = int(drowsiness_check)
                dispatch_data.loc[dispatch_data['ID'] == max_id, '말이 어눌해졌다'] = int(stammer_check)
